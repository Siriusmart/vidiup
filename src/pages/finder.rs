use actix_web::{body::BoxBody, get, http::header::ContentType, web::{Query, Redirect}, HttpRequest, HttpResponse, Responder};
use serde::Deserialize;

use crate::{RegionSelectorEntry, INSTANCES_RECORD, INTERFACE_CONFIG, OUTBOUND_CONFIG, POLLING_RECORD};
...
fn selector(selected: &Option<String>) -> String {
    let mut selectors = vec![ if selected.is_some() {r#"<a href="/finder">All regions</a>"#.to_string()} else {r#"<span>All regions</span>"#.to_string()}];

    for RegionSelectorEntry { display, internal } in INTERFACE_CONFIG.get().unwrap().regions_selector.iter() {
...
pub async fn finder(query: Query<GetQuery>, req: HttpRequest) -> HttpResponse {
    if query.region.as_ref().is_some_and(|region| !OUTBOUND_CONFIG.get().unwrap().offsets.0.contains_key(region)) {
        Redirect::to("/finder").permanent().respond_to(&req).set_body(BoxBody::new(""))
    } else {
        HttpResponse::Ok().content_type(ContentType::html()).body(finder_task(query).await)
    }
}
...
async fn finder_task(query: Query<GetQuery>) -> String {
    ...
    if OUTBOUND_CONFIG.get().unwrap().polling.features.video {
        ...
        if let Some(latency) = record.video {
            stats.push(format!(r#"<td class="{}">{}ms</td>"#, INTERFACE_CONFIG.get().unwrap().latency_thresholds.quality(latency), latency.saturating_add_signed(offset)));
        } else { 
    ...
    let html = format!(r#"<p id="subhead">
